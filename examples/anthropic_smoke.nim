...
let model: LanguageModel = anthropic(getEnv("ANTHROPIC_API_KEY")).model("claude-sonnet-4-6")
var chunks = 0
let response: ProviderResponse = streamText(model, proc (event: StreamEvent): bool =
...
let objectResponse: ObjectResult[json.JsonNode] = generateObject(model,
