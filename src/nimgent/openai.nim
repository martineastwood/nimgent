...
##
## OpenRouter's API is the same wire format plus a few extras (session_id,
## Anthropic-style cache_control, HTTP-Referer). Those stay optional on this
## type so both factories share generate / generateStream.
...
type
  OpenAIProvider* = ref object of Provider
    ...
    applyCache*: bool
    ## "max_tokens" (OpenRouter) or "max_completion_tokens" (OpenAI).
...
proc buildOpenAiBody*(request: ProviderRequest, stream: bool): JsonNode =
  ## Native OpenAI body: max_completion_tokens, no session_id or cache_control.
...
proc initChatProvider(name, displayName, apiKey, endpoint: string,
                      ...
                      includeSessionId = false, applyCache = false,
                      maxTokensField = "max_completion_tokens"): OpenAIProvider =
...
proc makeOpenAIProvider*(apiKey: string, endpoint = defaultOpenAiEndpoint,
  ...
  if "tool_calls" in message:
    for call in message["tool_calls"]:
      ...
      try:
        ...
        raiseProviderError(provider.label & " returned invalid tool arguments: " & e.msg)
