...
description: Monitor retries, tool timing, steps, and final usage.
---

Observe what happens during a multi-step generation run with `RunCallbacks`.

Each callback reports one part of the run: retries, tool start and finish,
completed model steps, or the final response. This is useful for logging,
metrics, and progress reporting without changing the tool implementation.
...
[View the source example](https://github.com/martineastwood/nimgent/blob/main/examples/lifecycle_callbacks.nim)
