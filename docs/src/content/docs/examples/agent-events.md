---
title: Agent events
description: Observe agent events and approve tool calls.
---

Watch an agent run through a pull-based event stream and decide whether a tool
may run.

The example defines a runtime JSON schema with `rawTool`, asks for approval
before `deleteFile` runs, and denies that request. Text, errors, and approval
events are handled while the run continues in the background.
...
[View the source example](https://github.com/martineastwood/nimgent/blob/main/examples/agent_events.nim)
